...
namespace SEP490_FTCDHMM_API.Application.Services.Implementations
{
    public class DraftRecipeService : IDraftRecipeService
    {
        ...
        public async Task UpdateDraftAsync(Guid userId, Guid draftId, DraftRecipeRequest request)
        {
            ...
            var oldImageId = draft.ImageId;
            var cleanupDraft = new DraftRecipe { Id = draft.Id, ImageId = draft.ImageId };
            ...
            await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                _unitOfWork.RegisterAfterCommit(async () =>
                {
                    if (oldImageId.HasValue)
                        await _imageRepository.MarkDeletedAsync(oldImageId);

                    await _imageRepository.MarkDeletedStepsImageFromDraftAsync(cleanupDraft);
                });
            });
        }
        ...
        public async Task<IEnumerable<DraftRecipeResponse>> GetDraftsAsync(Guid userId)
        {
            ...
            return _mapper.Map<IEnumerable<DraftRecipeResponse>>(draft);
        }
        ...
        public async Task<DraftDetailsResponse> GetDraftByIdAsync(Guid userId, Guid draftId)
        {
            ...
            return _mapper.Map<DraftDetailsResponse>(draft);
        }

        private static void ValidateDraftRequest(DraftRecipeRequest request)
        {
