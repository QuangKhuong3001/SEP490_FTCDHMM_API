...
{
    public class GetNutrientsAsyncTests : NutrientServiceTestBase
    {
        [Fact]
        public async Task GetNutrientsAsync_ShouldReturnMappedResult()
        {
            var n1 = CreateNutrient(Guid.NewGuid());
            var n2 = CreateNutrient(Guid.NewGuid());
            ...
            var result = await Sut.GetNutrientsAsync();
